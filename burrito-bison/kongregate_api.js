...
(function() {
    ...
    Kongregate.MessageConnection.prototype = {
        ...
        processMessage: function(a, c) {
            ...
            for (var d = 0; d < this._messageListeners.length; d++) this._messageListeners[d](a.opcode, a.params)
        },
...
(function() {
    ...
    Kongregate.AnalyticsServices.prototype = {
        ...
        addFilterType: function(a) {
            if (this._enabled && a) {
                var c = /^[a-zA-Z0-9_]+$/;
                !a || !c.test(a) ? Kongregate.Log.warn("Invalid filterType: " + a + ", must be alpha_numeric") : (c = (c = this._savedData[B]) ? c.split(",") : [], 0 > Kongregate.Utils.indexOf(c, a) && (c.push(a), c.sort(), this._savedData[B] = c.join(","), this._flushPersistentStore(), Kongregate.Log.info("Filter type added: " + a)))
            }
        },
        ...
        updateCommonProperties: function() {
            try {
                "function" === typeof this._commonPropsCallback && (this._commonProperties = this._objectify(this._commonPropsCallback()), this._savedData[ha] = this._commonProperties, this._commonProperties[O] && !this._savedData[C] && (this._savedData[C] = this._commonProperties[O]))
            } catch (a) {
        ...
        _flushQueue: function() {
            if (this._swrveClient) {
                var a = this;
                this._eventQueue.length && !this._submitLock && (Kongregate.Log.debug("Flushing swrve event queue"), this._submitLock = !0, this._swrveClient.sendEvents(this._eventQueue, function(c) {
                    a._submitLock = !1;
        ...
        _addIAPEvent: function(a, c, b) {
            ...
            n[T] = this._iapID;
            D != b && (n[U] = a || V);
            this._services.isKongregate() ? (n[K] = this._getKredPrice(this._productId), n[L] = "KRED") : (n[K] = c[K] || this._priceUSD, n[L] = c[L] || "USD");
        ...
        _processAnalyticsPayload: function(a, c) {
            ...
            if (this._enabled)
                if (this._checkBrowserSupport(c)) {
                    ...
                    })), this._swrveClient ? (this._submitLock = !1, this._startHeartbeatTimer(), this._flushPendingEvents(), this._sendSwrveUserEvent(), this.addEvent(Q, {}), this._flushQueue()) : (Kongregate.Log.debug("Analytics not initialized, no services configured"), this._enabled = !1))
...
KongregateAPI.prototype = {
    ...
    _processParam: function(a, b) {
        0 === a.indexOf("kongregate") && (this._flashVarsObject[a] = b, this._flashVarsString += a + "=" + b + "&")
    },
...
Kongregate.MicrotransactionServices.prototype = {
    ...
    _onPurchaseResultResponse: function(a) {
        this._purchaseItemsCallback && (this._purchaseItemsCallback(a), this._purchaseItemsCallback = null)
    },
...
Kongregate.SharedContentServices.prototype = {
    ...
    save: function(a, b, c, d, e) {
        if (this._saving || a.length > Kongregate.SharedContentServices.CONTENT_TYPE_LIMIT) c && c({
            success: !1
        });
        else {
            var k = {};
...
(function() {
    ...
    Kongregate.SwrveClient.prototype = {
        ...
        _pruneEvents: function(a) {
            for (var b = a.length - 1; 0 <= b; b--) a[b].event && a[b].event.retry_count > this._maxRetries() && (Kongregate.Log.debug("Event " + a[b].name + " is over max retry count, deleting it"), a.splice(b, 1));
